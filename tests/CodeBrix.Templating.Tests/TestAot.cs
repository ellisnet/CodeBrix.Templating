...
public class TestAot
{
    ...
    {
        // Find the CodeBrix.Templating.Tests.Aot project relative to the test assembly location
        var testDir = AppContext.BaseDirectory;
        var srcDir = Path.GetFullPath(Path.Combine(testDir, "..", "..", "..", ".."));
        var aotProjectDir = Path.Combine(srcDir, "CodeBrix.Templating.Tests.Aot");
        var aotProjectFile = Path.Combine(aotProjectDir, "CodeBrix.Templating.Tests.Aot.csproj");

        if (!File.Exists(aotProjectFile))
        {
            Assert.Skip($"CodeBrix.Templating.Tests.Aot project not found at {aotProjectFile}");
            return;
        }
        ...
        using var process = Process.Start(psi) ?? throw new Xunit.Sdk.XunitException("Expected process to start.");
