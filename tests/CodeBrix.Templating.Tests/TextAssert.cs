...
public static class TextAssert
{
    ...
    public static void AreEqual(string expected, string actual, DiffStyle diffStyle, TextWriter output)
    {
        ...
        // Normalize CRLF -> LF on both sides so that test data files checked out with
        // platform-specific line endings (e.g. CRLF on Windows via git autocrlf) compare
        // equal to the LF-only output produced by the CodeBrix.Templating library.
