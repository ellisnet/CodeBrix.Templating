...
public class TestLexer
{
    ...
    public void ParseNumberInvalid()
    {
        ...
        Assert.Contains("Expecting at least one digit after the exponent", lexer.Errors.First().Message);
    }
    ...
    {
        {
            ...
            var tokens = lexer.ToList();
            Assert.True(lexer.HasErrors);
            Assert.Contains("Unexpected } while no matching", lexer.Errors.First().Message);
        }
    }
    ...
    {
        {
            ...
            var tokens = lexer.ToList();
            Assert.True(lexer.HasErrors);
            Assert.Contains("Unexpected hex number", lexer.Errors.First().Message);
        }
        {
        ...
        }
        {
            ...
            var tokens = lexer.ToList();
            Assert.True(lexer.HasErrors);
            Assert.Contains("Unexpected end of file while parsing a string not terminated", lexer.Errors.First().Message);
        }
        {
            var lexer = new Lexer("{{ `");
            var tokens = lexer.ToList();
            Assert.True(lexer.HasErrors);
            Assert.Contains("Unexpected end of file while parsing a verbatim string not terminated by", lexer.Errors.First().Message);
        }
    }
