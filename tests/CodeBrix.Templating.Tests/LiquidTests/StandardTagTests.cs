...
{%endfor%}";
    ...
    public void TestAssignFromCase()
    {
        ...
        Assert.Equal("menswear", template.Render(Hash.FromAnonymousObject(new { collection = new { handle = "menswear-jackets" } })));
        Assert.Equal("menswear", template.Render(Hash.FromAnonymousObject(new { collection = new { handle = "menswear-t-shirts" } })));
        Assert.Equal("womenswear", template.Render(Hash.FromAnonymousObject(new { collection = new { handle = "x" } })));
        Assert.Equal("womenswear", template.Render(Hash.FromAnonymousObject(new { collection = new { handle = "y" } })));
        Assert.Equal("womenswear", template.Render(Hash.FromAnonymousObject(new { collection = new { handle = "z" } })));
    }
    ...
    {
        Assert.Equal("variable", Template.ParseLiquid("{% assign a = 'variable' %}{{a}}").Render());
    }

    [Fact]
    public void TestAssignAnEmptyString()
    {
        Assert.Equal("", Template.ParseLiquid("{% assign a = '' %}{{a}}").Render());
    }

    [Fact]
    public void TestAssignIsGlobal()
    {
        Assert.Equal("variable", Template.ParseLiquid("{%for i in (1..2) %}{% assign a = 'variable'%}{% endfor %}{{a}}").Render());
    }
